...
pub fn part_one(input: &str) -> Option<u32> {
    let values = input
        ...
        .map(|line| {
            ...
            format!("{}{}", digits_in_line.first().unwrap(), digits_in_line.last().unwrap())
        })
...
pub fn part_two(input: &str) -> Option<u32> {
    let string_digits: Vec<&str> = Vec::from(["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]);
    ...
    let values = input.lines().map(|line| {
        ...
        let first_digit = &digit_locations.iter().min_by(|(_, pos1), (_, pos2)| pos1.cmp(pos2));
        let last_digit = &digit_locations.iter().max_by(|(_, pos1), (_, pos2)| pos1.cmp(pos2));

        format!("{}{}", first_digit.unwrap().0, last_digit.unwrap().0)
    }).map(|digit| digit.parse::<u32>().unwrap()).collect::<Vec<u32>>();
